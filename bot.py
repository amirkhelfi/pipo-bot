    ...
    try:
        if arg.startswith('https://t.me/') or arg.startswith('t.me/'):
            if 'joinchat' in arg or '+' in arg:
                return await event.reply("❌ لا يمكن للبوت الانضمام عبر رابط دعوة خاص. استخدم @username لمجموعة عامة.")
            else:
            ...
            except: return await event.reply("❌ صيغة غير صحيحة.")
...
async def leave_group_handler(event):
    ...
    try:
        await client.send_message(gid, f"🚨 **BAAY** 🚨\n🛑 أنا طالع من المجموعة بأمر من المطور.\n👑 **المطور:** @{DEVELOPER_USERNAME}\n🛡️ PIPO BOT")
...
async def enable_auto_lock(event):
    if not is_admin(await event.get_sender()): return
    bot_settings["auto_lock_enabled"] = True; save_settings()
...
async def disable_auto_lock(event):
    if not is_admin(await event.get_sender()): return
    bot_settings["auto_lock_enabled"] = False; save_settings()
...
async def auto_lock_unlock():
    ...
    while True:
        ...
        if h == 0 and m == 0 and not chat_locked:
            chat_locked = True
            for gid in active_groups:
                try: await client.edit_permissions(gid, send_messages=False)
                except: pass
        if h == 10 and m == 0 and chat_locked:
            chat_locked = False
            for gid in active_groups:
                try: await client.edit_permissions(gid, send_messages=True)
                except: pass
        await asyncio.sleep(30)
...
async def set_welcome_media(event):
    ...
    try:
        if hasattr(media, 'photo') and media.photo:
            ...
            typ = 'video'
        else: return await event.reply("❌ يجب أن تكون صورة أو فيديو.")
...
async def all_commands(event):
    txt = (
        ...
        "/تفعيل_القفل_التلقائي - /تعطيل_القفل_التلقائي\n\n"
        "**👤 الأعضاء:**\n"
        "/start - /ايدي - /حب - /سر\n"
